...
public class DBAccessBlockingInterceptor implements HandlerInterceptor, InitializingBean, BeanFactoryAware {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (DBAccessBlockingRequestHandler.class.isAssignableFrom(handlerMethod.getBeanType())) {
			...
			// block 되어 있는 테이블이 하나라도 있으면 block 상태이다.
			Set<String> blockedTables = this.blockQueryManager.getBlockedTables();
			BLOCKED = CollectionUtils.isEmpty(blockedTables) == false;
	...
	public void afterPropertiesSet() throws Exception {
		...
		beanDefinitionRegistry.registerBeanDefinition("blockedQueryExceptionResolver",
			blockedQueryExceptionResolverBeanDefinitionBuilder.getBeanDefinition());
	}
	...
	private static class BlockedQueryExceptionResolver implements HandlerExceptionResolver, Ordered {
		 ...
		 * @param ex
		 */
		private void redirectToBlockedAccess(HttpServletRequest request, HttpServletResponse response, Object handler,
			BlockedQueryExecutionException ex) {
