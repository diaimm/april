...
package com.diaimm.april.db.jpa.hibernate.multidbsupport;
...
/**
 * defaultDataSource 이외에 dataSourceLookup으로 부터 persistenceUnitKey를 통해 datasource를 얻어올 수 있게 하기 위해
 *
...
public class DefaultPersistenceUnitManager implements PersistenceUnitManager, ResourceLoaderAware, LoadTimeWeaverAware, InitializingBean {
	...
	private static final TypeFilter[] entityTypeFilters = new TypeFilter[] {new AnnotationTypeFilter(Entity.class, false),
	...
	public void preparePersistenceUnitInfos() {
		...
		for (SpringPersistenceUnitInfo pui : puis) {
			...
			if (pui.getNonJtaDataSource() == null) {
				if (this.getDataSourceLookup().getDataSource(pui.getPersistenceUnitName()) != null) {
					pui.setNonJtaDataSource(this.getDataSourceLookup().getDataSource(pui.getPersistenceUnitName()));
				} else {
	...
	public PersistenceUnitInfo obtainDefaultPersistenceUnitInfo() {
		...
		if (this.persistenceUnitInfos.isEmpty()) {
			throw new IllegalStateException("All persistence units from " + ObjectUtils.nullSafeToString(this.persistenceXmlLocations)
				+ " already obtained");
		}
